...
import { deleteUser, getUserById, getUsers, registerUser, updateUser } from './user-queries'
    ...
    onError: (error) => {
      if (axios.isAxiosError(error) && !error.response) {
        toast.error('Backend unavailable', {
          description:
            'Could not reach the local backend. If this is a packaged build, the embedded backend may have failed to start. Restart the app and try again.'
        })
        return
      }
